...
// 1. Initial State update (ensure mikrotikName is in the form)
const targetFormInit = /name: '',/g;
if (content.match(targetFormInit)) {
    content = content.replace(targetFormInit, "name: '',\n          mikrotikName: '',");
    console.log("Form State updated");
}

// 2. Update Form UI
// Find the Display Name field block and replace it with TWO columns (Display Name + MikroTik Name)
const targetFormUI = /<div className="col-span-2">\s*<label className="text-xs font-bold text-slate-500 mb-1 flex items-center">\s*\{isRTL \? 'اسم الخدمة \(Profile Name\)' : 'Profile Name'\}\s*<InfoTooltip text=\{isRTL \? 'الاسم الذي سيظهر في الميكروتيك ولوحة التحكم' : 'The internal name shown in MikroTik and NetLink'\} \/>\s*<\/label>\s*<input type="text" value=\{profileForm\.name\} onChange=\{\(e\) => setProfileForm\(\{\.\.\.profileForm, name: e\.target\.value\}\)\} className="w-full mt-1 bg-white dark:bg-\[#18181B\] border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 focus:border-teal-500 text-slate-800 dark:text-slate-100" \/>\s*<\/div>/;

const replacementFormUI = `                                     <div className="col-span-2 md:col-span-1">
...
if (targetFormUI.test(content)) {
    content = content.replace(targetFormUI, replacementFormUI);
...
// 3. Update Table UI
const targetTableUI = /<td className="p-4">\s*<div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">\s*\{p\.name\}\s*<span className="text-\[10px\] bg-slate-200 dark:bg-slate-700 px-2 py-0\.5 rounded text-slate-500 dark:text-slate-400 uppercase">\{p\.type\}<\/span>\s*<\/div>\s*<\/td>/;

const replacementTableUI = `<td className="p-4">
...
if (targetTableUI.test(content)) {
    content = content.replace(targetTableUI, replacementTableUI);
